...
if (! function_exists('active_class_url')) {
    /**
     * Get the active class if the condition is not falsy.
     *
     * @param        $condition
     ...
     * @return string
     */
    function active_class_url($uri, $activeClass = 'active', $inactiveClass = '')
    {
        if(is_array($uri)){
            $isValid = false;

            foreach ($uri as $url) {
                if(str_contains(url()->current(), $url)) $isValid = true;
            }

            return $isValid ? $activeClass : $inactiveClass;
        } else {
            return str_contains(url()->current(), $uri) ? $activeClass : $inactiveClass;
        }
    }
}
