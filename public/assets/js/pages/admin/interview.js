    ...
    var dt = ppdbTable.dataTable({
        ...
        columnDefs: [{
            ...
            render: function(data, type, row) {
                ...
                if(row.deleted_at != null && row.deleted_at != ''){
                    ...
                    <button type="button" class="btn btn-danger btn-sm w-100">
                        <i class="fa-solid fa-user-times"></i> Tidak Lulus
                    </button>`;
                } else {
                    if(row.interview_result == 1) {

                        if(row.is_submited == 1) {                        
                            return `
                            <button type="button" class="btn btn-success btn-sm w-100">
                                <i class="fa-solid fa-user-check"></i> Lulus
                            </button>`;    
                        } else {
                            return `
                            <button type="button" class="btn btn-light-success btn-sm w-100">
                                <i class="fa-solid fa-user-check"></i> Konfirmasi Sekolah
                            </button>`;
                        }

                    } else if (row.interview_result == 2) {

                        if(row.is_submited == 1) {                        
                            return `
                            <button type="button" class="btn btn-danger btn-sm w-100">
                                <i class="fa-solid fa-user-xmark"></i> Tidak Lulus
                            </button>`;   
                        } else {
                            return `
                            <button type="button" class="btn btn-light-danger btn-sm w-100">
                                <i class="fa-solid fa-user-xmark"></i> Konfirmasi Sekolah
                            </button>`;
                        }
                    } else if (row.interview_result == 3) {
                        return `
                        <button type="button" class="btn btn-warning btn-sm w-100">
                            <i class="fa-solid fa-users"></i> Dipertimbangkan
                        </button>`;
                    } else {
                        if(row.school_recomendation_result > 0){
                            return `
                            <button type="button" class="btn btn-info btn-sm w-100">
                                <i class="fa-solid fa-user-pen"></i> Review BPS
                            </button>`;
                        }
                        
                        return `
                        <button type="button" class="btn btn-secondary btn-sm w-100">
                            <i class="fa-solid fa-users"></i> New
                        </button>`;
                    }
                }
            }
            ...
            render: function(data, type, row) {
                ...
                if(is_interviewer && (row.school_recomendation_result > 0 && row.interview_result == 0) ){
                    return ``;
                }

                if(is_rnd && (row.school_recomendation_result < 1) ){
                    return ``;
                }
