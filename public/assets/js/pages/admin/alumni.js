...
var uri_edit_dapodik = $('input[name="uri_edit_dapodik"]').val();
...
FTX.Utils.documentReady(function() {
    ...
    var dt = ppdbTable.dataTable({
        ...
        render: function(data, type, row) {
            ...
            render: function(data, type, row) {
                if(row.sekolah !=null){
                return `<span class="text d-block fs-8">` + row.sekolah + `</span>`;
        } else{
            ...
            render: function(data, type, row) {
                if(row.unit !=null){
                return `<span class="text d-block fs-8">` + row.unit + `</span>`;
            } else{
            ...
            render: function(data, type, row) {
                ...
                return `<span class="text d-block fs-8">`+row.kelas_utama+`</span>`;
            } else{
                return `<span class="btn btn-sm w-30 fs-9 btn-dark ppdb-bg-status-not-have-class">`+ row.kelas_utama + `</span>`;
        }
            }
        },  {
            targets: 6,
            ...
            render: function(data, type, row) {
                ...
                console.log(row);               
                return `<span class="btn btn-sm w-30 fs-9 btn-warning">`+row.status_siswa+`</span>`;
            } else{
            ...
            render: function(data, type, row) {
                ...
                console.log(row);
                return `<span class="btn btn-sm w-30 fs-9 btn-warning">`+row.keterangan+`</span>`;
            } else{
            ...
            render: function(data, type, row) {
                var row_cool = '';
                if(row.ppdb_id == null) {
                    var uri_target = uri_edit_dapodik.replace("::target::", row.dapodik_id);
                        return `
                        <a href="` + uri_target + `" class="btn btn-sm btn-warning border w-lg-60 w-100"> 
